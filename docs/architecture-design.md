...
| Per-cluster Prometheus/Loki, ephemeral storage | Managed observability SaaS (Grafana Cloud, Amazon Managed Prometheus) | Cheaper to self-host at small scale; the trade-off flips at real scale, where self-hosting the aggregation tier becomes its own HA service to maintain |
